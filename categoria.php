<?php
/**
 * PHP PAGE THAT LISTS THE FILMS OF THE SELECTED CATEGORY
 */
...
//mysql_select_db('nopubfilm') or die("SELEZIONE DATABASE FALLITA");
  ...
  if(strcmp($ordine,"anno") == 0) {
    $query = "SELECT DISTINCT nome, link,Trama,locandina,genere,funzionante,sito,voto,votanti,imdb,anno FROM Altadefinizione WHERE genere = '$categoria' ORDER BY anno DESC";
  }
  if(strcmp($ordine,"nome") == 0) {
    $query = "SELECT DISTINCT nome, link,Trama,locandina,genere,funzionante,sito,voto,votanti,imdb,anno FROM Altadefinizione WHERE genere = '$categoria' ORDER BY nome";
  }
  if(strcmp($ordine,"voto") == 0){
    $query = "SELECT DISTINCT nome, link,Trama,locandina,genere,funzionante,sito,voto,votanti,imdb,anno FROM Altadefinizione WHERE genere = '$categoria' ORDER BY imdb DESC" ;
  }
...
<div id='divAlto'> <a href=''><img src='/img/categorie/<?php echo $categoria; ?>.jpg'></a> </div>
...
<div class="row">
  ...
  <div class="col-md-6">
    <form action="" method="get">
      Ordina:
      <div class="btn-group">
          <a class="btn btn-default <?php if(strcmp($ordine,'nome')==0){echo 'active';} ?>" href="/categoria.php?categoria=<?php echo $categoria; ?>&ordine=nome">Per Titolo</a>
          <a class="btn btn-default <?php if(strcmp($ordine,'anno')==0){echo 'active';} ?>" href="/categoria.php?categoria=<?php echo $categoria; ?>&ordine=anno">Per Anno</a>
          <a class="btn btn-default <?php if(strcmp($ordine,'voto')==0){echo 'active';} ?>" href="/categoria.php?categoria=<?php echo $categoria; ?>&ordine=voto">Per Voto</a>
      </div>
    </form>
  </div>
</div>
...
<div id='header'>
  <br>
  <br> Utilizza CTRL+F (cerca) per cercare il film desiderato <br>
...
<div id='film'>
  ...
  while($r = mysql_fetch_array($result, MYSQL_ASSOC)){
    $titolo = $r['nome'];
    $link = $r['link'];
    //cambio il nome alla locandina da .black a .cool
    $locandina_NOEDIT = $r['locandina'];
    $locandina_pre = substr($locandina_NOEDIT,0,29);
    $locandina_post = substr($locandina_NOEDIT, 34);
    if(strcmp(substr($locandina_post,0,1), "/") != 0){
      $locandina_post = "/".substr($locandina_NOEDIT, 34);
    }
    $locandina = $locandina_pre."cool".$locandina_post;
    $voto = $r['imdb'];
    $anno = $r['anno'];

    printf("<div id='$titolo' class='divCategoria'><a href='schedaFilm.php?link=%s&titolo=%s'><img width='175' height='246' src='%s'></a><div class='anno'><span><a href='ricercaanno.php?anno=%s'>%s</a></span></div><div class='voto'><span> VOTO: %s/10</span></div><div class='titolo'><span>%s</span></div></div>", $link,$titolo,$locandina,$anno,$anno,$voto,$titolo);
