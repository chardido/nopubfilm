<?php
/**
 * THIS IS A SIMPLE SEARCH PAGE THAT SEARCH FOR THE FILM DUE TO THE NAME OF THE ACTOR GIVEN
 */
...
//mysql_select_db('nopubfilm') or die("SELEZIONE DATABASE FALLITA");
  ...
  if (strcmp($ordine, "anno") == 0) {
    $query = "SELECT DISTINCT nome, link,Trama,locandina,genere,funzionante,sito,voto,votanti,imdb,anno,attore FROM Altadefinizione WHERE attore LIKE '%$ricercata%' ORDER BY anno DESC";
  }
  if (strcmp($ordine, "nome") == 0) {
    $query = "SELECT DISTINCT nome, link,Trama,locandina,genere,funzionante,sito,voto,votanti,imdb,anno,attore FROM Altadefinizione WHERE attore LIKE '%$ricercata%' ORDER BY nome";
  }
  if (strcmp($ordine, "voto") == 0) {
    $query = "SELECT DISTINCT nome, link,Trama,locandina,genere,funzionante,sito,voto,votanti,imdb,anno,attore FROM Altadefinizione WHERE attore LIKE '%$ricercata%' ORDER BY imdb DESC";
  }
  ...
  mysql_query("SET NAMES 'utf8'");
  mysql_query('SET CHARACTER SET utf8');
  $query = "SELECT DISTINCT nome,link,trama,locandina,funzionante,sito,voto,votanti,imdb,anno FROM Altadefinizione WHERE attore LIKE '%$ricercata%' ORDER BY nome;";
  $result = mysql_query($query) or die("QUERY FALLITA");
  $i = 0;
}
?>
  ...
  <div class="header">
    ...
    <nav class="wsmenu slideLeft clearfix">
      <ul class="mobile-sub wsmenu-list">
        ...
        <li><span class="wsmenu-click"><i class="wsmenu-arrow fa fa-angle-down"></i></span><a href="#" class="active"><i class="fa fa-align-justify"></i>&nbsp;&nbsp;Categorie <span class="arrow"></span><!--<span class="highlighter">23</span>--></a>
...
<div class="row">
  ...
  <div class="col-md-6">
    <form action="" method="get">
      Ordina:
      <div class="btn-group">
          <a class="btn btn-default <?php if(strcmp($ordine,'nome')==0){echo 'active';} ?>" href="/ricercaattore.php?ordine=nome&nome=<?php echo $ricercata; ?>">Per Titolo</a>
          <a class="btn btn-default <?php if(strcmp($ordine,'anno')==0){echo 'active';} ?>" href="/ricercaattore.php?ordine=anno&nome=<?php echo $ricercata; ?>">Per Anno</a>
          <a class="btn btn-default <?php if(strcmp($ordine,'voto')==0){echo 'active';} ?>" href="/ricercaattore.php?ordine=voto&nome=<?php echo $ricercata; ?>">Per Voto</a>
      </div>
    </form>
  </div>
</div>
<div class="header"> <br>Risultati della ricerca con parole chiave: <span><?php echo $ricercata; ?><span> </div><br>
