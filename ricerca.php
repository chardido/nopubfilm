<?php
/**
 * THIS IS A SIMPLE SEARCH PAGE THAT SEARCH FOR THE FILM DUE TO THE NAME OF THE FILM GIVEN
 */
if(isset($_GET['ordine'])){
  $ricercata = $_GET['ricercata'];
  $ordine = $_GET['ordine'];
...
//mysql_select_db('nopubfilm') or die("SELEZIONE DATABASE FALLITA");
  ...
  if(strcmp($ordine,"anno") == 0) {
    $query = "SELECT DISTINCT nome, link,Trama,locandina,genere,funzionante,sito,voto,votanti,imdb,anno FROM Altadefinizione WHERE nome LIKE \"%$ricercata%\" ORDER BY anno DESC";
  }
  if(strcmp($ordine,"nome") == 0) {
    $query = "SELECT DISTINCT nome, link,Trama,locandina,genere,funzionante,sito,voto,votanti,imdb,anno FROM Altadefinizione WHERE nome LIKE \"%$ricercata%\" ORDER BY nome";
  }
  if(strcmp($ordine,"voto") == 0){
    $query = "SELECT DISTINCT nome, link,Trama,locandina,genere,funzionante,sito,voto,votanti,imdb,anno FROM Altadefinizione WHERE nome LIKE \"%$ricercata%\" ORDER BY imdb DESC" ;
  }
  $result = mysql_query($query) or die("QUERY FALLITA");
  $i = 0;
}else {
  $ricercata = $_POST['nome'];
  $conn = mysql_connect('localhost', 'nopubfilm', '') or die("CONNESSIONE DATABASE FALLITA");
  mysql_select_db('my_nopubfilm') or die("SELEZIONE DATABASE FALLITA");
  mysql_query("SET NAMES 'utf8'");
  mysql_query('SET CHARACTER SET utf8');
  $query = "SELECT DISTINCT nome,link,trama,locandina,funzionante,sito,voto,votanti,imdb,anno FROM Altadefinizione WHERE nome LIKE \"%$ricercata%\" ORDER BY nome;";
  $result = mysql_query($query) or die("QUERY FALLITA");
  $i = 0;
}
?>
...
<div class="row">
  ...
  <div class="col-md-6">
    <form action="" method="get">
      Ordina:
      <div class="btn-group">
          <a class="btn btn-default <?php if(strcmp($ordine,'nome')==0){echo 'active';} ?>" href="/ricerca.php?ordine=nome&ricercata=<?php echo $ricercata; ?>">Per Titolo</a>
          <a class="btn btn-default <?php if(strcmp($ordine,'anno')==0){echo 'active';} ?>" href="/ricerca.php?ordine=anno&ricercata=<?php echo $ricercata; ?>">Per Anno</a>
          <a class="btn btn-default <?php if(strcmp($ordine,'voto')==0){echo 'active';} ?>" href="/ricerca.php?ordine=voto&ricercata=<?php echo $ricercata; ?>">Per Voto</a>
      </div>
    </form>
  </div>
</div>
